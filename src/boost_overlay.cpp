...
static LRESULT CALLBACK Hooked_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  if (s_visible.load(std::memory_order_relaxed)) {
    ...
    if (io.WantCaptureMouse || io.WantCaptureKeyboard) {
      // Suppress WM_KEYDOWN / WM_KEYUP / mouse clicks from GD while overlay is active
      switch (msg) {
...
static void saveCurrentSettingsToDisk() {
  ...
  ini.set("BoostCocos", "particle_throttle", cfg.particle_throttle ? "true" : "false");
  ini.set("BoostCocos", "particle_max", std::to_string(cfg.particle_max));
  ini.set("BoostGD", "skip_shake_flash", cfg.skip_shake_flash ? "true" : "false");

  ini.set("BoostLatency", "frame_pacing", cfg.frame_pacing ? "true" : "false");
  ini.set("BoostLatency", "frame_pacing_target", std::to_string(cfg.frame_pacing_target));

  ini.set("BoostAdvanced", "gl_state_dedup", cfg.gl_state_dedup ? "true" : "false");
  ini.set("BoostRender", "mipmap_off", cfg.mipmap_off ? "true" : "false");
  ini.set("BoostRenderAdv", "disable_aa", cfg.disable_aa ? "true" : "false");

  ini.set("BoostStutterMonitor", "stutter_monitor", cfg.stutter_monitor ? "true" : "false");
...
static void checkHotkeyToggle() {
  ...
  if (std::chrono::duration_cast<std::chrono::milliseconds>(now - s_lastToggleTime).count() < 250) {
    return;
  }
  ...
  if (isAltPressed && isHomePressed) {
    s_visible.store(!s_visible.load(std::memory_order_relaxed), std::memory_order_relaxed);
    s_lastToggleTime = now;
    angle::log("boost_overlay: toggled -> %s", s_visible.load() ? "VISIBLE" : "HIDDEN");
  }
}
...
void render(HDC hdc) {
  ...
  if (!s_initialized.load(std::memory_order_relaxed)) {
    ...
    s_origWndProc = (WNDPROC)SetWindowLongPtrA(s_hwnd, GWLP_WNDPROC, (LONG_PTR)Hooked_WndProc);
  ...
  if (ImGui::Begin("⚡ ReviANGLE Studio (In-Game Configurator)", nullptr, ImGuiWindowFlags_NoCollapse)) {

    ImGui::TextColored(ImVec4(0.35f, 0.64f, 1.00f, 1.00f), "⚡ Live Tuning - Settings apply instantly without restarting!");
    ImGui::Separator();
    ImGui::Spacing();

    if (ImGui::CollapsingHeader("📊 HUD & Statistics", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::CollapsingHeader("✨ Cocos2d & Particles (Live)", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      ImGui::Checkbox("Skip Screen Flash & Camera Shake", &cfg.skip_shake_flash);
    }

    if (ImGui::CollapsingHeader("🎯 Latency & Frame Pacing (Live)", ImGuiTreeNodeFlags_DefaultOpen)) {
      ImGui::Checkbox("QPC High-Precision Frame Pacer", &cfg.frame_pacing);
      if (cfg.frame_pacing) {
        ImGui::SliderInt("Target FPS Cap (0 = Uncapped)", &cfg.frame_pacing_target, 0, 360);
      }
    }

    if (ImGui::CollapsingHeader("🎨 Render Optimizations (Live)", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::CollapsingHeader("🔍 Stutter Monitor (Live)")) {
    ...
    if (ImGui::Button("💾 Save Settings to angle_config.ini", ImVec2(240, 32))) {
    ...
    auto now = std::chrono::steady_clock::now();
    if (!s_statusMessage.empty() && std::chrono::duration_cast<std::chrono::seconds>(now - s_statusTime).count() < 4) {
      ImGui::Spacing();
      ImGui::TextColored(ImVec4(0.40f, 0.85f, 0.40f, 1.00f), "%s", s_statusMessage.c_str());
    }
  }
  ImGui::End();
