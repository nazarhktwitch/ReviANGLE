...
namespace boost_unlock_fps { bool isActive(); }
...
HGLRC WINAPI wgl_wglCreateContext(HDC hdc) {
    ...
    EGLint_t ctxAttribsNoError[] = {
        EGL_CONTEXT_CLIENT_VERSION,         3,
        EGL_CONTEXT_OPENGL_NO_ERROR_KHR,    1,
        EGL_NONE,                           EGL_NONE
    };
    EGLint_t ctxAttribsBasic[] = {
        EGL_CONTEXT_CLIENT_VERSION,         3,
        EGL_NONE,                           EGL_NONE
    };
...
extern "C" HGLRC WINAPI wgl_wglCreateContextAttribsARB(HDC hdc, HGLRC shareContext, const int* attribList);
static int WINAPI wgl_wglGetSwapIntervalEXT() { return 0; }
static const char* WINAPI wgl_wglGetExtensionsStringEXT() {
    return "WGL_EXT_swap_control WGL_ARB_extensions_string WGL_EXT_extensions_string WGL_ARB_create_context WGL_ARB_create_context_profile";
}
...
PROC WINAPI wgl_wglGetProcAddress(LPCSTR name) {
    ...
    if (name[0] == 'w' && name[1] == 'g' && name[2] == 'l') {
        if (!strcmp(name, "wglCreateContextAttribsARB")) return (PROC)wgl_wglCreateContextAttribsARB;
    ...
    if (name[0] == 'g' && name[1] == 'l' && selfMod) {
        for (auto& s : kSuffixes) {
            if (nameLen > s.len && !strcmp(name + nameLen - s.len, s.suffix)) {
                ...
                if (p) {
                    static int san = 0;
                    if (san < 240) { angle::log("wglGPA SELF alias %s -> %s -> %p", name, base, p); san++; }
                    return p;
                }
            }
        }
